...
def predict_model(test_dataloader, model, device):
    ...
    with torch.no_grad():
        # Add tqdm to the dataloader to show a progress bar
        for (his_input_title, pred_input_title), _ in tqdm(test_dataloader, desc="Predicting batches"):
            his_input_title = his_input_title.to(device)
            pred_input_title = pred_input_title.to(device)
