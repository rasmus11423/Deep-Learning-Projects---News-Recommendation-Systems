...
def compute_normalized_distribution(
    ...
    Args:
        R (NDArray[np.str_]): An array of item representations.
        weights (Optional[NDArray[np.float64]]): Weights for each element in `R`. Defaults to uniform weights.
        distribution (Optional[dict[str, float]]): Existing distribution to update. Defaults to a new dictionary.
